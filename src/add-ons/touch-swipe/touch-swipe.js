...
class TouchSwipe {
    ...
    updateHandler() {
        ...
        if ( this.accordion.getTotalPages() > 1 ) {
            this.accordion.panelsContainerEl.classList.add( 'ga-grab' );
        } else {
            this.accordion.panelsContainerEl.classList.remove( 'ga-grab' );
        }
    }
    ...
    add() {
        ...
        Array.from( this.accordion.panelsContainerEl.getElementsByTagName( 'a' ) ).forEach( ( linkEl ) => {
            linkEl.addEventListener( 'touchend', this.eventHandlerReferences[ 'touchend.link' ] = () => {
                const parentPanel = getParent( linkEl, 'ga-panel' );

                if ( parentPanel !== null && parentPanel.classList.contains( 'ga-opened' ) === false ) {
    ...
    touchStartHandler( event ) {
        ...
        if ( event.target.classList.contains( 'ga-selectable' ) ) {
            return;
        }
        ...
        // Swap grabbing icons
        this.accordion.panelsContainerEl.classList.replace( 'ga-grab', 'ga-grabbing' );
    }
    ...
    touchMoveHandler( event ) {
        ...
        // Add 'ga-swiping' class to indicate that the slides are being swiped
        if ( this.accordion.accordionEl.classList.contains( 'ga-swiping' ) === false ) {
            this.accordion.accordionEl.classList.add( 'ga-swiping' );
        }
    ...
    touchEndHandler() {
        ...
        // Swap grabbing icons
        this.accordion.panelsContainerEl.classList.replace( 'ga-grabbing', 'ga-grab' );
        ...
        if ( this.accordion.accordionEl.classList.contains( 'ga-swiping' ) ) {
            setTimeout(() => {
                this.accordion.accordionEl.classList.remove( 'ga-swiping' );
            }, 100 );
        }
