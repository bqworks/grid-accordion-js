...
class Video {
    ...
    updateHandler() {
        ...
        Array.from( this.accordion.accordionEl.querySelectorAll( '.ga-video:not(a):not([data-video-init])' ) ).forEach(( videoEl ) => {
            this.initVideo( videoEl );
        });

        // Find all the lazy-loaded videos and pre-initialize them. They will be initialized
        // only when their play button is clicked.
        Array.from( this.accordion.accordionEl.querySelectorAll( 'a.ga-video:not([data-video-preinit])' ) ).forEach(( videoEl ) => {
    ...
    preinitVideo( videoEl ) {
        ...
        this.preinitVideoClickHandler = ( event ) => {
            ...
            if ( this.accordion.accordionEl.classList.contains( 'ga-swiping' ) || 
                videoEl.parentElement.querySelector( '.ga-video[data-video-init]' ) !== null ) {
                return;
            }
    ...
    panelOpenHandler( event ) {
        ...
        const previousVideoEl = this.accordion.panelsContainerEl.getElementsByClassName( 'ga-panel' )[ event.detail.previousIndex ].querySelector( '.ga-video[data-video-init]' );
        ...
        if ( previousVideo !== null ) {
            if ( this.settings.closePanelVideoAction === 'stopVideo' ) {
                ...
                if ( previousVideoEl.parentElement.querySelector( 'a.ga-video' ) !== null ) {

                    previousVideoEl.parentElement.querySelector( 'a.ga-video' ).style.removeProperty( 'display' );
    ...
    panelOpenCompleteHandler( event ) {
        ...
        if ( this.settings.openPanelVideoAction === 'playVideo' && event.detail.index === this.accordion.getCurrentIndex() ) {
            const loadedVideoEl = this.accordion.panelsContainerEl.getElementsByClassName( 'ga-panel' )[ event.detail.index ].querySelector( '.ga-video[data-video-init]' ),
                unloadedVideoEl = this.accordion.panelsContainerEl.getElementsByClassName( 'ga-panel' )[ event.detail.index ].querySelector( '.ga-video[data-video-preinit]' );
    ...
    // Destroy the module
    destroy() {
        Array.from( this.accordion.accordionEl.querySelectorAll( '.ga-video[ data-video-preinit ]' ) ).forEach( ( videoEl ) => {
        ...
        Array.from( this.accordion.accordionEl.querySelectorAll( '.ga-video[ data-video-init ]' ) ).forEach( ( videoEl ) => {
            ...
            if ( videoEl.parentElement.querySelector( '.ga-video[ data-video-preinit ]' ) !== null ) {
                videoEl.remove();
            }
        });
