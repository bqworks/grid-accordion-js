    ...
    init() {
        ...
        this.accordion.addEventListener( 'panelOpen.' + this.namespace, ( event ) => {
            ...
            const panel = this.accordion.getPanelAt( event.detail.index ),
                background = panel.panelEl.getElementsByClassName( 'ga-background' )[ 0 ],
                opened = panel.panelEl.getElementsByClassName( 'ga-background-opened' )[ 0 ];
            ...
            if ( event.detail.previousIndex !== -1 && event.detail.index !== event.detail.previousIndex ) {
                ...
                const previousPanel = this.accordion.getPanelAt( event.detail.previousIndex ),
                    previousBackground = previousPanel.panelEl.getElementsByClassName( 'ga-background' )[ 0 ],
                    previousOpened = previousPanel.panelEl.getElementsByClassName( 'ga-background-opened' )[ 0 ];
