class DeepLinking {
    ...
    init() {
        ...
        this.accordion.addEventListener( 'panelOpen.' + this.namespace, ( event ) => {
            if ( this.settings.updateHash === true ) {
                ...
                let panelId = this.accordion.accordionEl.getElementsByClassName( 'ga-panel' )[ event.detail.index ].getAttribute( 'id' );
    ...
    gotoHash( hash ) {
        ...
        if ( isNaN( panelIdNumber ) ) {
            ...
            const panelIndex = Array.from( this.accordion.accordionEl.getElementsByClassName( 'ga-panel') ).indexOf( panelEl );
