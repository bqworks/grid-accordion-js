...
describe( 'video add-on', () => {
    describe( 'inline videos', () => {
        beforeAll( () => {
            ...
            videoEl = accordionEl.getElementsByClassName( 'ga-video' )[ 0 ];
            videoMock = new VideoElementMock( videoEl );
        });
    
        test( 'should initialize inline videos that have the `ga-video` class', () => {
            Array.from( accordionEl.querySelectorAll( '.ga-video:not(a)' ) ).forEach( videoEl => {
                expect( videoEl.getAttribute( 'data-video-init' ) ).not.toBe( null );
            });
        });
    
        test( 'should pre-initialize lazy-loaded videos that have the `ga-video` class', () => {
            Array.from( accordionEl.querySelectorAll( 'a.ga-video' ) ).forEach( videoEl => {
        ...
        test( 'should destroy videos when the accordion is destroyed', () => {
            ...
            expect( accordion.accordionEl.querySelectorAll( '.ga-video[data-video-init]' ).length ).toBe( 0 );
            expect( accordion.accordionEl.querySelectorAll( '.ga-video[data-video-preinit]' ).length ).toBe( 0 );
        });
    });
    ...
    describe( 'lazy loaded videos', () => {
        ...
        test( 'should initialize lazy loaded youtube videos on video click', () => {
            ...
            const lazyVideoEl = panelEl.getElementsByClassName( 'ga-video' )[ 0 ];
        ...
        test( 'should remove the lazy loaded video when the accordion navigates to a new slide', () => {
            accordion.openPanel( 3 );

            accordion.settings.closePanelVideoAction = 'removeVideo';
            accordion.update();

            let videoEl = accordion.getPanelAt( 3 ).panelEl.querySelector( '.ga-video[data-video-init]' );

            expect( videoEl ).not.toBe( null );

            accordion.nextPanel();

            videoEl = accordion.getPanelAt( 3 ).panelEl.querySelector( '.ga-video[data-video-init]' );
